...
private:
	...
	void ChangeMode();
	void ChangeCT(NodeVatTu *dsvt, DSNhanVien &dsnv);
	void EscCT();
public:
	MainProject();
	~MainProject();
...
void MainProject::ChangeCT(NodeVatTu *dsvt, DSNhanVien &dsnv)
{
...
//			WriteCenterText("DANH SACH NHAN VIEN", 80, 5, 0);
		...
		case MainProject::HoaDon:
		{
			...
			if(choose == 1){
//				ShowCur(1);
				int posNV = printdsNhanVien(dsnv,dsnv,1);
//				string curNV = "";
//				curNV = searchEngine(root,dsnv);
//				int posNV = SeachNV(dsnv, curNV);
				if(posNV != -1){
					ThemMoiHoaDon(dsnv.DsNhanVien[posNV], dsvt,dsnv);
//					WriteDataVatTu(dsvt);
